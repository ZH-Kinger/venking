...
console.log(
  "\n✅ 本地闭环完成。commit & push 到 venking 即触发 CI:构建 → GitHub Pages + rsync 增量推服务器 + 原子切换。" +
    "\n   ⚠️ 服务器博客只由 CI 部署(基线统一),别本地直推 nginx,否则会污染基线拖慢下次 CI。" +
    "\n   线上 RAG(AI 知识库)生效仍需:cd rag-server && SERVER=<user@host> bash deploy.sh(推新 chroma)。",
);
